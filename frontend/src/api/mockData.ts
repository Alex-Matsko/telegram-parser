...
const productDefs: ProductDef[] = [
  ...
  { id: 22, name: 'Apple Watch Series 9 45mm Midnight Aluminum', brand: 'Apple', model: 'Apple Watch Series 9', memory: null, color: 'Midnight', condition: 'new', basePrice: 37900, currency: 'RUB' },
  { id: 23, name: 'Apple Watch Series 9 41mm Starlight Aluminum', brand: 'Apple', model: 'Apple Watch Series 9', memory: null, color: 'Starlight', condition: 'new', basePrice: 34900, currency: 'RUB' },
...
// Generate stable mock data (seeded by calling once)
let _priceListCache: PriceListItem[] | null = null;
function getPriceList(): PriceListItem[] {
  if (!_priceListCache) {
    _priceListCache = productDefs.map(generatePriceListItem);
  }
...
export function mockGetProductOffers(productId: number): { product: Product; offers: Offer[] } {
  const pDef = productDefs.find(p => p.id === productId) || productDefs[0];
  const numOffers = rnd(3, 6);
  const offers: Offer[] = Array.from({ length: numOffers }, (_, i) => {
    ...
    return {
      ...
      detected_confidence: 0.85 + Math.random() * 0.15,
      is_current: true,
      created_at: h(rnd(1, 48)),
  ...
  const product: Product = {
    id: pDef.id,
    category: pDef.model.includes('iPhone') ? 'smartphone' : pDef.model.includes('AirPods') ? 'headphones' : pDef.model.includes('Watch') ? 'watch' : 'laptop',
    brand: pDef.brand,
    line: pDef.model.split(' ')[0],
    model: pDef.model,
    generation: null,
    memory: pDef.memory,
    color: pDef.color,
    ...
    sku_key: `${pDef.brand}/${pDef.model}/${pDef.memory || '-'}/${pDef.color || '-'}/${pDef.condition}`,
...
export function mockGetPriceHistoryChart(productId: number, days: number = 3): PriceHistoryChartData {
  const pDef = productDefs.find(p => p.id === productId) || productDefs[0];
...
export function mockGetPriceChangeEvents(productId: number, days: number = 3): PriceChangeEvent[] {
  const pDef = productDefs.find(p => p.id === productId) || productDefs[0];
...
export const mockSources: Source[] = [
  {
    id: 1, type: 'channel', telegram_id: -1001234567890, source_name: 'Apple Price Moscow',
    supplier_id: 1, is_active: true, poll_interval_minutes: 15, parsing_strategy: 'auto',
    bot_scenario_id: null, last_read_at: h(0.5), last_message_id: 45230,
    error_count: 0, last_error: null, created_at: '2026-03-01T10:00:00Z', updated_at: h(0.5),
  },
  {
    id: 2, type: 'channel', telegram_id: -1001234567891, source_name: 'iStore Прайс',
    supplier_id: 2, is_active: true, poll_interval_minutes: 30, parsing_strategy: 'regex',
    bot_scenario_id: null, last_read_at: h(1), last_message_id: 12450,
    error_count: 2, last_error: 'FloodWaitError: 30 seconds', created_at: '2026-03-01T12:00:00Z', updated_at: h(1),
  },
  {
    id: 3, type: 'group', telegram_id: -1001234567892, source_name: 'Гаджеты оптом',
    supplier_id: 3, is_active: true, poll_interval_minutes: 30, parsing_strategy: 'auto',
    bot_scenario_id: null, last_read_at: h(2), last_message_id: 87652,
    error_count: 0, last_error: null, created_at: '2026-03-02T10:00:00Z', updated_at: h(2),
  },
  {
    id: 4, type: 'bot', telegram_id: 1234567893, source_name: 'ТехноМаркет Бот',
    supplier_id: 4, is_active: true, poll_interval_minutes: 60, parsing_strategy: 'llm',
    bot_scenario_id: 1, last_read_at: h(3), last_message_id: 334,
    error_count: 0, last_error: null, created_at: '2026-03-02T14:00:00Z', updated_at: h(3),
  },
  {
    id: 5, type: 'channel', telegram_id: -1001234567894, source_name: 'Digital Zone Prices',
    supplier_id: 5, is_active: true, poll_interval_minutes: 15, parsing_strategy: 'regex',
    bot_scenario_id: null, last_read_at: h(0.3), last_message_id: 67890,
    error_count: 0, last_error: null, created_at: '2026-03-03T10:00:00Z', updated_at: h(0.3),
  },
  {
    id: 6, type: 'channel', telegram_id: -1001234567895, source_name: 'МСК Apple Цены',
    supplier_id: 6, is_active: true, poll_interval_minutes: 30, parsing_strategy: 'auto',
    bot_scenario_id: null, last_read_at: h(4), last_message_id: 23456,
    error_count: 5, last_error: 'Connection timeout after 30s', created_at: '2026-03-03T12:00:00Z', updated_at: h(4),
  },
  {
    id: 7, type: 'bot', telegram_id: 1234567896, source_name: 'СПБ Прайс Бот',
    supplier_id: 7, is_active: false, poll_interval_minutes: 60, parsing_strategy: 'auto',
    bot_scenario_id: 2, last_read_at: '2026-03-15T10:00:00Z', last_message_id: 112,
    error_count: 12, last_error: 'Bot blocked by user', created_at: '2026-03-04T10:00:00Z', updated_at: '2026-03-15T10:00:00Z',
  },
  {
    id: 8, type: 'group', telegram_id: -1001234567897, source_name: 'Apple Trade Group',
    supplier_id: null, is_active: true, poll_interval_minutes: 45, parsing_strategy: 'llm',
    bot_scenario_id: null, last_read_at: h(1.5), last_message_id: 99123,
    error_count: 1, last_error: 'Rate limit reached, retrying in 60s', created_at: '2026-03-05T10:00:00Z', updated_at: h(1.5),
  },
];
...
export const mockUnresolvedMessages: UnresolvedMessage[] = [
  {
    id: 1, source_id: 2, source_name: 'iStore Прайс', telegram_message_id: 12444,
    message_text: '15pm 256 nat esim — 91500\n15pm 512 bt esim — 108к\n15p 128 wt — 79к',
    message_date: h(5), sender_name: null, parse_status: 'needs_review',
    parse_error: 'Множественные товары в одном сообщении, низкая уверенность разбора',
    suggested_product: 'iPhone 15 Pro Max 256GB Natural Titanium', suggested_product_id: 8,
    created_at: h(5),
  },
  {
    id: 2, source_id: 3, source_name: 'Гаджеты оптом', telegram_message_id: 87640,
    message_text: 'AW Ultra 2 nat — 68к\nкоробки на месте, eSIM',
    message_date: h(8), sender_name: 'Андрей', parse_status: 'needs_review',
    parse_error: 'Неоднозначная модель: AW Ultra 2 может быть Apple Watch Ultra 2',
    suggested_product: 'Apple Watch Ultra 2', suggested_product_id: 24,
    created_at: h(8),
  },
  {
    id: 3, source_id: 1, source_name: 'Apple Price Moscow', telegram_message_id: 45215,
    message_text: 'Обновление: все модели +2-3% с завтра, успевайте по старым ценам!',
    message_date: h(12), sender_name: null, parse_status: 'failed',
    parse_error: 'Не удалось извлечь товар и цену: информационное сообщение',
    suggested_product: null, suggested_product_id: null,
    created_at: h(12),
  },
  {
    id: 4, source_id: 6, source_name: 'МСК Apple Цены', telegram_message_id: 23440,
    message_text: '16/256 black esim 101000\n16/512 black esim 118000\n16pm/256 nat 123000',
    message_date: h(4), sender_name: null, parse_status: 'needs_review',
    parse_error: 'Разобрано частично: 16pm/256 nat — не удалось определить модель (16 Pro Max?)',
    suggested_product: 'iPhone 16 Pro Max 256GB Natural Titanium', suggested_product_id: 1,
    created_at: h(4),
  },
  {
    id: 5, source_id: 8, source_name: 'Apple Trade Group', telegram_message_id: 99100,
    message_text: 'MBA M3 13 256 mid — 105к\nMBA M3 15 256 sg — 125к\nпод заказ 2-3 дня',
    message_date: h(6), sender_name: 'Дмитрий', parse_status: 'needs_review',
    parse_error: 'Сокращения: MBA = MacBook Air, mid = Midnight?, sg = Space Gray?',
    suggested_product: 'MacBook Air M3 13" 256GB Midnight', suggested_product_id: 25,
    created_at: h(6),
  },
  {
    id: 6, source_id: 2, source_name: 'iStore Прайс', telegram_message_id: 12430,
    message_text: 'Фото: [image]\nAP Pro 2 — 17500\nAP Max midnight — 49900',
    message_date: h(14), sender_name: null, parse_status: 'failed',
    parse_error: 'Не удалось извлечь: AP Pro 2 — неоднозначная модель (AirPods Pro 2?)',
    suggested_product: null, suggested_product_id: null,
    created_at: h(14),
  },
  {
    id: 7, source_id: 5, source_name: 'Digital Zone Prices', telegram_message_id: 67880,
    message_text: 'MBP M3 Pro 14 512 silver — 195к\nв наличии 3шт',
    message_date: h(10), sender_name: null, parse_status: 'needs_review',
    parse_error: 'Частичный разбор: MBP M3 Pro = MacBook Pro M3 Pro?',
    suggested_product: 'MacBook Pro M3 Pro 14" 512GB Silver', suggested_product_id: 29,
    created_at: h(10),
  },
  {
    id: 8, source_id: 4, source_name: 'ТехноМаркет Бот', telegram_message_id: 330,
    message_text: 'Ошибка бота: "Сервис временно недоступен, попробуйте позже"',
    message_date: h(20), sender_name: 'bot', parse_status: 'failed',
    parse_error: 'Сервисное сообщение бота, не содержит данных о ценах',
    suggested_product: null, suggested_product_id: null,
    created_at: h(20),
  },
];
...
export function mockGetUnresolved(filters: { status?: string; page?: number; per_page?: number }): UnresolvedResponse {
  ...
  if (filters.status) {
    items = items.filter(m => m.parse_status === filters.status);
  }
